...
const STYLE = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hero { min-height: 100vh; display: flex; align-items: center; padding: 120px 5vw 80px; position: relative; overflow: hidden; }
  ...
  .hero-eyebrow { display: inline-flex; align-items: center; gap: 0.5rem; background: var(--copper-pale); color: var(--copper); padding: 0.4rem 1rem; border-radius: 100px; font-size: 0.8rem; font-weight: 600; letter-spacing: 0.05em; text-transform: uppercase; margin-bottom: 1.5rem; }
  .hero-eyebrow::before { content: ''; width: 6px; height: 6px; background: var(--copper); border-radius: 50%; }
  .hero-headline { font-family: 'Fraunces', serif; font-size: clamp(2.8rem, 5vw, 4.2rem); font-weight: 400; line-height: 1.1; letter-spacing: -0.02em; color: var(--charcoal); margin-bottom: 1.5rem; }
  .hero-headline em { font-style: italic; color: var(--copper); }
  .hero-sub { font-size: 1.1rem; color: var(--warm-gray); line-height: 1.75; max-width: 520px; margin-bottom: 2.5rem; font-weight: 400; }
  .hero-actions { display: flex; gap: 1rem; align-items: center; flex-wrap: wrap; }
  .hero-proof { display: flex; align-items: center; gap: 0.75rem; margin-top: 3rem; padding-top: 2rem; border-top: 1px solid var(--clay); color: var(--warm-gray); font-size: 0.85rem; }
  .avatars { display: flex; }
  .avatar { width: 36px; height: 36px; border-radius: 50%; border: 2px solid var(--cream); margin-left: -8px; display: flex; align-items: center; justify-content: center; font-size: 0.75rem; font-weight: 600; color: white; }
  ...
  .stat-l { font-size: 0.8rem; color: rgba(255,255,255,0.55); letter-spacing: 0.03em; margin-top: 0.25rem; display: block; }
  ...
  .h2 { font-family: 'Fraunces', serif; font-size: clamp(2rem, 4vw, 3rem); font-weight: 400; line-height: 1.15; letter-spacing: -0.02em; color: var(--charcoal); max-width: 600px; margin-bottom: 1rem; }
  .h2.lt { color: white; }
  .sub { font-size: 1.05rem; color: var(--warm-gray); line-height: 1.7; max-width: 560px; margin-bottom: 3.5rem; }
  .sub.lt { color: rgba(255,255,255,0.6); }

  .steps-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 2rem; }
  .step-card { background: white; border: 1px solid var(--clay); border-radius: 20px; padding: 2.5rem 2rem; transition: all 0.3s ease; }
  ...
  .step-t { font-family: 'Fraunces', serif; font-size: 1.35rem; font-weight: 500; color: var(--charcoal); margin-bottom: 0.75rem; line-height: 1.3; }
  .step-d { color: var(--warm-gray); font-size: 0.95rem; line-height: 1.7; }
  ...
  .diff-c { padding: 2rem 1.75rem; border-radius: 20px; border: 1px solid rgba(255,255,255,0.08); background: rgba(255,255,255,0.04); }
  ...
  .blog-card { background: white; border: 1px solid var(--clay); border-radius: 20px; overflow: hidden; cursor: pointer; transition: all 0.3s ease; }
    ...
    <>
      ...
      <section className="hero">
        ...
        <div className="hero-content">
          ...
          <p className="hero-sub">We help local businesses integrate AI into their everyday workflows so they save time, respond faster, and make more money without hiring more people.</p>
          ...
          <div className="hero-proof">
            <div className="avatars">
              {["#B5622A","#5C7A4E","#2A6B7C","#6B4E8C"].map((c, i) => (
                <div key={i} className="avatar" style={{ background: c }}>{["JM","SR","KP","AL"][i]}</div>
              ))}
            </div>
            <span>Trusted by service businesses across the country</span>
          </div>
        </div>
        <div style={{ position: "absolute", right: "5vw", top: "50%", transform: "translateY(-50%)", pointerEvents: "none", opacity: 0.9 }}>
          <svg width="390" height="390" viewBox="0 0 420 420" fill="none">
            <circle cx="210" cy="210" r="180" fill="#E8DDD0" opacity="0.4"/>
            <circle cx="210" cy="210" r="130" fill="#D4C5B0" opacity="0.45"/>
            <circle cx="210" cy="210" r="80" fill="#B5622A" opacity="0.12"/>
            {[0,60,120,180,240,300].map((deg, i) => {
              const rad = deg * Math.PI / 180;
              const x = 210 + 155 * Math.cos(rad), y = 210 + 155 * Math.sin(rad);
              const ix = 210 + 55 * Math.cos(rad), iy = 210 + 55 * Math.sin(rad);
              return (
                <g key={i}>
                  <line x1={ix} y1={iy} x2={x} y2={y} stroke="#B5622A" strokeWidth="1" strokeDasharray="4 4" opacity="0.28"/>
                  <circle cx={x} cy={y} r="26" fill="white" opacity="0.9"/>
                  <text x={x} y={y+7} textAnchor="middle" fontSize="18">{["📱","📧","📅","💬","📊","🔔"][i]}</text>
                </g>
              );
            })}
            <circle cx="210" cy="210" r="52" fill="white" opacity="0.95"/>
            <text x="210" y="204" textAnchor="middle" fontSize="22">⚡</text>
          </svg>
        </div>
      </section>
      ...
      <section className="section" id="automations">
        <div className="inner">
          ...
          <p className="sub">Browse automations by niche. Every system is custom-built with no templates and no off-the-shelf tools that do not quite fit.</p>
          <div className="tabs">
            {NICHES.map((n, i) => (
      ...
      <section className="section section-dark">
        <div className="inner">
          ...
          <p className="sub lt" style={{ marginBottom: "3rem" }}>Most automation consultants build workflows. We build systems that actually solve your business problem, and we measure success in dollars and hours saved.</p>
          <div className="diff-grid">
            {[
              ...
              { i:"📐", t:"Industry Expertise", d:"Deep knowledge in real estate, local services, and agencies. We know your pain points before you describe them." },
      ...
      <section className="section section-alt" id="blog">
        <div className="inner">
          ...
          <div className="blog-grid">
            {BLOG_POSTS.map((p,i) => (
              <a key={i} href={`/blog/${p.slug}`} className="blog-card" style={{ textDecoration: "none" }}>
        ...
        <div className="contact-grid">
          ...
          <div className="cform">
            ...
            <div className="fg">
              ...
              <textarea placeholder="What's the one thing eating the most time in your business right now? Where do things fall through the cracks? What tasks do you wish you could hand off?" value={form.bottleneck} onChange={e => setForm(f => ({...f, bottleneck: e.target.value}))} />
            </div>
